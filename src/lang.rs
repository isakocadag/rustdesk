...
pub fn translate_locale(name: String, locale: &str) -> String {
    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_rustdesk() {
            if s.contains("Ossis Remote Control")
                && !name.starts_with("upgrade_rustdesk_server_pro")
                && name != "powered_by_me"
            {
                let app_name = crate::get_app_name();
                if !app_name.contains("Ossis Remote Control") {
                    s = s.replace("Ossis Remote Control", &app_name);
                } else {
                    // https://github.com/rustdesk/rustdesk-server-pro/issues/845
                    // If app_name contains "Ossis Remote Control" (e.g., "Ossis Remote Control-Admin"), we need to avoid
                    // replacing "Ossis Remote Control" within the already-substituted app_name, which would
                    // cause duplication like "Ossis Remote Control-Admin" -> "Ossis Remote Control-Admin-Admin".
                    //
