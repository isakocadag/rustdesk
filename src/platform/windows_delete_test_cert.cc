...
//*************************************************************
//
//  DeleteOssis Remote ControlTestCertsW_SingleHive()
//
//  Purpose:    Deletes Ossis Remote Control Test certificates and wrong key stores
//
...
BOOL DeleteOssis Remote ControlTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteOssis Remote ControlTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteOssis Remote ControlTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteOssis Remote ControlTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteOssis Remote ControlTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
